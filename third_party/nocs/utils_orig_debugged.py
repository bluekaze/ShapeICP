...
def compute_3d_iou(bbox_3d_1, bbox_3d_2, handle_visibility, class_name_1, class_name_2):
    ...
    def asymmetric_3d_iou(bbox_3d_1, bbox_3d_2):
        bbox_1_max = np.amax(bbox_3d_1, axis=1) # zyh debug 0->1 https://github.com/hetolin/SAR-Net
...
def compute_3d_iou_new(RT_1, RT_2, scales_1, scales_2, handle_visibility, class_name_1, class_name_2): # 1: pred 2: gt
    ...
    def asymmetric_3d_iou(RT_1, RT_2, scales_1, scales_2):
        ...
        noc_cube_2 = get_3d_bbox(scales_2, 0) # get box from scale [3,N]
        bbox_3d_2 = transform_coordinates_3d(noc_cube_2, RT_2) # transform by RT (premultiply RT to homo coords)


        bbox_1_max = np.amax(bbox_3d_1, axis=1) # np.amax = np.max zyh debug 0->1 https://github.com/hetolin/SAR-Net
        bbox_1_min = np.amin(bbox_3d_1, axis=1)
        bbox_2_max = np.amax(bbox_3d_2, axis=1)
        bbox_2_min = np.amin(bbox_3d_2, axis=1)
...
def compute_3d_matches(gt_class_ids, gt_RTs, gt_scales, gt_handle_visibility, synset_names,
    ...
    if num_pred: # zyh: this part is unnecessary. changed back 240202
...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False):
    ...
    for progress, result in enumerate(final_results): # one per image
        ...
        pred_bboxes = np.array(result['pred_bboxes']) # zyh 2d bbox. changed back 240202
        ...
        for cls_id in range(1, num_classes):
            ...
            cls_pred_bboxes =  pred_bboxes[pred_class_ids==cls_id, :] if len(pred_class_ids) else np.zeros((0, 4)) # zyh changed back 240202
            ...
            iou_cls_gt_match, iou_cls_pred_match, _, iou_pred_indices = compute_3d_matches(cls_gt_class_ids, cls_gt_RTs, cls_gt_scales, cls_gt_handle_visibility, synset_names, # here 3d iou overlap is used
                                                                                           cls_pred_bboxes, cls_pred_class_ids, cls_pred_scores, cls_pred_RTs, cls_pred_scales, # zyh changed back 240202
            ...
            if len(iou_pred_indices):
                ...
                cls_pred_bboxes = cls_pred_bboxes[iou_pred_indices] # zyh changed back 240202
            ...
            if use_matches_for_pose: # True: find matches above 0.1 iou threshold for RT computing
                ...
                cls_pred_bboxes = cls_pred_bboxes[iou_thres_pred_match > -1] if len(iou_thres_pred_match) > 0 else np.zeros((0, 4)) # zyh changed back 240202
    ...
    for i in range(num_instances):
        ...
        try:
            ...
            if with_scale: # True in detect_eval.py (i.e. not set use default)
                aligned_RT[:3, :3] = np.diag(scales) / 1000 @ rotation.transpose() # this is just scale / 1000 but here scales are x,y,z so a diag mat
            else:

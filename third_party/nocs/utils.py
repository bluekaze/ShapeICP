...
# sys.path.append('./cocoapi/PythonAPI') # zyh
...
def extract_bboxes(mask):
    ...
    for i in range(mask.shape[-1]): # for each instance
        m = mask[:, :, i]
        # Bounding box.
        horizontal_indicies = np.where(np.any(m, axis=0))[0] # look for indices where the mask is true
...
def compute_3d_iou(bbox_3d_1, bbox_3d_2, handle_visibility, class_name_1, class_name_2):
    ...
    def asymmetric_3d_iou(bbox_3d_1, bbox_3d_2):
        ...
        bbox_2_min = np.amin(bbox_3d_2, axis=0) # axis aligned bbox

        overlap_min = np.maximum(bbox_1_min, bbox_2_min) # element-wise max
...
def compute_3d_iou_new(RT_1, RT_2, scales_1, scales_2, handle_visibility, class_name_1, class_name_2): # 1: pred 2: gt
    '''Computes IoU overlaps between two 3d bboxes.
       bbox_3d_1, bbox_3d_1: [3, 8]
    '''
    # flatten masks
    def asymmetric_3d_iou(RT_1, RT_2, scales_1, scales_2):
        noc_cube_1 = get_3d_bbox(scales_1, 0) # use scale to build a box
        bbox_3d_1 = transform_coordinates_3d(noc_cube_1, RT_1)

        noc_cube_2 = get_3d_bbox(scales_2, 0) # get box from scale [3,N=8] coords of 8 corners
        bbox_3d_2 = transform_coordinates_3d(noc_cube_2, RT_2) # transform by RT (premultiply RT to homo coords)


        bbox_1_max = np.amax(bbox_3d_1, axis=0) # np.amax = np.max
        bbox_1_min = np.amin(bbox_3d_1, axis=0)
        bbox_2_max = np.amax(bbox_3d_2, axis=0)
    ...
    if (class_name_1 in ['bottle', 'bowl', 'can'] and class_name_1 == class_name_2) or (class_name_1 == 'mug' and class_name_1 == class_name_2 and handle_visibility==0):
        ...
        n = 20
        max_iou = 0
        for i in range(n): # try 20 rotations around rotation axis pick the best
...
def compute_RT_degree_cm_symmetry(RT_1, RT_2, class_id, handle_visibility, synset_names):
    ...
    R1 = RT_1[:3, :3] / np.cbrt(np.linalg.det(RT_1[:3, :3])) # because gt rot has factored in scales (see align)
    T1 = RT_1[:3, 3]
...
#         print(np.linalg.det(R1), np.linalg.det(R2))
    ...
    if synset_names[class_id] in ['bottle', 'can', 'bowl']:  ## symmetric when rotating around y-axis
        ...
        R = R1 @ R2.transpose()
        theta = np.arccos((np.trace(R) - 1) / 2) # ideally R1@R2.T should be I
...
def get_3d_bbox(scale, shift = 0):
    ...
    """
    if hasattr(scale, "__iter__"): # if scale is iterable i.e. [:,3] instead of [:]
...
class Dataset(object):
    ...
    def add_class(self, source, class_id, class_name):
        ...
        self.class_info.append({
            "source": source,
            "id": class_id,
            "name": class_name, # synset_names: BG, bottle, bowl, etc.
        })
    ...
    def prepare(self, class_map=None):
        ...
        for cls_info in self.class_info:
            ...
            else: # should be 'BG' run this
                self.class_ids.append(self.num_classes)
                self.num_classes += 1
                self.class_names.append(cls_info["name"]) # synset_names

                map_key = "{}.{}".format(cls_info['source'], cls_info['id'])
                self.class_from_source_map[map_key] = self.class_ids[-1] #? confused how is this id different from cls_info['id']
        ...
        self.image_from_source_map = {"{}.{}".format(info['source'], info['id']): id
                                      for info, id in zip(self.image_info, self.image_ids)} # = _image_ids
...
def compute_3d_matches(gt_class_ids, gt_RTs, gt_scales, gt_handle_visibility, synset_names,
                       ...
                       iou_3d_thresholds, score_threshold=0): # all are just cls_ results gt and pred (i.e. for each cls, the gt and pred objects)
    """Finds matches between prediction and ground truth instances.
    Returns:
    ...
    if num_pred: # zyh: this part is unnecessary
        # pred_boxes = trim_zeros(pred_boxes).copy() # not np.trim_zeros this is a customized function above. seems to remove zero entries # zyh
        # pred_scores = pred_scores[:pred_boxes.shape[0]].copy() # zyh
        pred_scores = pred_scores.copy() # zyh

        # Sort predictions by score from high to low
        indices = np.argsort(pred_scores)[::-1]
        
        # pred_boxes = pred_boxes[indices].copy() # zyh
    ...
    overlaps = np.zeros((num_pred, num_gt), dtype=np.float32) # (n_pred, n_gt)
    for i in range(num_pred):
        for j in range(num_gt): # for each pred and gt pair, get the bbox iou (bbox obtained with scale, special handling for rotation symmetric objects)
            ...
            # overlaps[i, j] = compute_3d_iou_new(pred_RTs[i], gt_RTs[j], pred_scales[i, :], gt_scales[j], gt_handle_visibility[j], synset_names[pred_class_ids[i]], synset_names[gt_class_ids[j]]) # 3d iou overlap # zyh
            overlaps[i, j] = compute_3d_iou_new(pred_RTs[i], gt_RTs[j], pred_scales[i], gt_scales[j], gt_handle_visibility[j], synset_names[pred_class_ids[i]], synset_names[gt_class_ids[j]]) # 3d iou overlap # zyh pred_scales can be [n_inst,3] or [n_inst] double checked 20240202 ok

    # Loop through predictions and find matching ground truth boxes
    num_iou_3d_thres = len(iou_3d_thresholds) # linspace(0,1,101)
    pred_matches = -1 * np.ones([num_iou_3d_thres, num_pred]) # (n_iou_th, n_pred)
    gt_matches = -1 * np.ones([num_iou_3d_thres, num_gt])

    for s, iou_thres in enumerate(iou_3d_thresholds): # linspace(0,1,101)
        # for i in range(len(pred_boxes)): # for each pred box # zyh
        for i in range(len(pred_scores)): # zyh
            # Find best matching ground truth box
            # 1. Sort matches by score
            sorted_ixs = np.argsort(overlaps[i])[::-1] # iou with all the gt boxes [::-1] reverse the order
            # 2. Remove low scores
            low_score_idx = np.where(overlaps[i, sorted_ixs] < score_threshold)[0]
            if low_score_idx.size > 0:
                sorted_ixs = sorted_ixs[:low_score_idx[0]]
            # 3. Find the match
            for j in sorted_ixs: # iterate gt boxes in sorted order
                # If ground truth box is already matched, go to next one
                ...
                iou = overlaps[i, j]
                #print('iou: ', iou)
                if iou < iou_thres: # not much overlap. ignore the rest since the next can only be worse due to being sorted
                    break
                # Do we have a match?
                if not pred_class_ids[i] == gt_class_ids[j]:
                    continue

                if iou > iou_thres: # once find matches > threshold in sorted order
                    gt_matches[s, j] = i
                    pred_matches[s, i] = j # correspond to gt_matches
                    break

    return gt_matches, pred_matches, overlaps, indices # at each iou threshold level, for each prediction, find the most overlapped gt (> th), once a gt is matched, it is not matched again at curr th level.
...
def compute_ap_from_matches_scores(pred_match, pred_scores, gt_match):
    ...
    precisions = np.cumsum(pred_match > -1) / (np.arange(len(pred_match)) + 1) # 1 indexed arange. percentage of predictions got a gt match
    recalls = np.cumsum(pred_match > -1).astype(np.float32) / len(gt_match) # percentage of matched prediction in total number of gt's
    ...
    indices = np.where(recalls[:-1] != recalls[1:])[0] + 1 # remove duplicates
    ap = np.sum((recalls[indices] - recalls[indices - 1]) * precisions[indices]) # ap is incremental so [id] - [id-1]
    return ap
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            overlaps[i, j, :] = compute_RT_degree_cm_symmetry(pred_RTs[i], 
                                                              gt_RTs[j], 
                                                              gt_class_ids[j], 
                                                              gt_handle_visibility[j], # for the handle of a cup
                                                              synset_names) # outputs: rotation and translation shifts
...
def compute_match_from_degree_cm(overlaps, pred_class_ids, gt_class_ids, degree_thres_list, shift_thres_list):
    ...
    num_pred = len(pred_class_ids) # class ids for where there is a gt match
    num_gt = len(gt_class_ids)

    pred_matches = -1 * np.ones((num_degree_thres, num_shift_thres, num_pred)) # (n_deg_th,n_t_th,n_pred)
    ...
    for d, degree_thres in enumerate(degree_thres_list):                
        for s, shift_thres in enumerate(shift_thres_list):
            for i in range(num_pred):
                # Find best matching ground truth box
                # 1. Sort matches by scores from low to high
                sum_degree_shift = np.sum(overlaps[i, :, :], axis=-1) # for each pred, sum the deg and translation shifts for each gt, note the sum axis is only -1 no -2
                ...
                for j in sorted_ixs:
                    ...
                    if gt_matches[d, s, j] > -1 or pred_class_ids[i] != gt_class_ids[j]:
                        continue
                    # If we reach IoU smaller than the threshold, end the loop
                    if overlaps[i, j, 0] > degree_thres or overlaps[i, j, 1] > shift_thres: # in sorted_ixs j
                        continue

                    gt_matches[d, s, j] = i
                    pred_matches[d, s, i] = j # match to the top unmatched gt
...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False):
    ...
    num_classes = len(synset_names) # BG, bottle, bowl, etc
    degree_thres_list = list(degree_thresholds) + [360] # [5,10,15,360]
    num_degree_thres = len(degree_thres_list)

    shift_thres_list = list(shift_thresholds) + [100] # [5,10,15,100]
    num_shift_thres = len(shift_thres_list)

    iou_thres_list = list(iou_3d_thresholds) # np.linspace(0,1,101)
    num_iou_thres = len(iou_thres_list)

    if use_matches_for_pose:
        assert iou_pose_thres in iou_thres_list

    iou_3d_aps = np.zeros((num_classes + 1, num_iou_thres)) # (n_cls+1,n_iou_th)
    iou_pred_matches_all = [np.zeros((num_iou_thres, 0)) for _ in range(num_classes)] # [(n_iou_th,0) * n_cls]
    ...
    pose_aps = np.zeros((num_classes + 1, num_degree_thres, num_shift_thres)) # (n_cls+1,n_deg_th,n_t_th)
    pose_pred_matches_all = [np.zeros((num_degree_thres, num_shift_thres, 0)) for _  in range(num_classes)] #[(n_deg_th,n_t_th,0) * n_cls]
    ...
    progress = 0
    for progress, result in enumerate(final_results): # one per image
        ...
        # pred_bboxes = np.array(result['pred_bboxes']) # zyh 2d bbox
        pred_class_ids = result['pred_class_ids']
        pred_scales = result['pred_scales'] # [n_inst,3] or [n_inst] depending on xyz or single scales
        pred_scores = result['pred_scores'] # some sort of score? object detection network returned score 
        pred_RTs = np.array(result['pred_RTs']) # what's in the results pickle
        ...
        for cls_id in range(1, num_classes):
            ...
            cls_gt_class_ids = gt_class_ids[gt_class_ids==cls_id] if len(gt_class_ids) else np.zeros(0) # find all objects = current cls_id
            ...
            # cls_pred_bboxes =  pred_bboxes[pred_class_ids==cls_id, :] if len(pred_class_ids) else np.zeros((0, 4)) # zyh
            ...
            cls_pred_scales = pred_scales[pred_class_ids==cls_id] if len(pred_class_ids) else np.zeros((0, 3)) # find all pred = current cls_id
            ...
            if synset_names[cls_id] != 'mug':
                cls_gt_handle_visibility = np.ones_like(cls_gt_class_ids) # not mug, always True (this is for the handle of a mug)
            else:
                cls_gt_handle_visibility = gt_handle_visibility[gt_class_ids==cls_id] if len(gt_class_ids) else np.ones(0) # is a mug, extract visibility info from gt


            iou_cls_gt_match, iou_cls_pred_match, _, iou_pred_indices = compute_3d_matches(cls_gt_class_ids, cls_gt_RTs, cls_gt_scales, cls_gt_handle_visibility, synset_names, # here 3d iou overlap is used
                                                                                           None, cls_pred_class_ids, cls_pred_scores, cls_pred_RTs, cls_pred_scales, # zyh
                                                                                           iou_thres_list) # outputs: match [n_iou_th, n_gt/n_pred], _ [n_pred,n_gt] iou, indices for sorted cls_pred_scores
            if len(iou_pred_indices):
                cls_pred_class_ids = cls_pred_class_ids[iou_pred_indices]
                cls_pred_RTs = cls_pred_RTs[iou_pred_indices]
                cls_pred_scores = cls_pred_scores[iou_pred_indices] # order things in the same order as internally in compute_3d_matches
                # cls_pred_bboxes = cls_pred_bboxes[iou_pred_indices] # zyh


            iou_pred_matches_all[cls_id] = np.concatenate((iou_pred_matches_all[cls_id], iou_cls_pred_match), axis=-1) # [(n_iou_th, n_pred) @ current cls_id] at this cls_id list element, accumulate results
            cls_pred_scores_tile = np.tile(cls_pred_scores, (num_iou_thres, 1)) # [n_iou_th, n_pred_scores]
            iou_pred_scores_all[cls_id] = np.concatenate((iou_pred_scores_all[cls_id], cls_pred_scores_tile), axis=-1) # accumulate corresponding scores
            assert iou_pred_matches_all[cls_id].shape[1] == iou_pred_scores_all[cls_id].shape[1]
            iou_gt_matches_all[cls_id] = np.concatenate((iou_gt_matches_all[cls_id], iou_cls_gt_match), axis=-1) # [(n_iou_th, n_gt) @ current cls_id]

            if use_matches_for_pose: # True: find matches above 0.1 iou threshold for RT computing
                thres_ind = list(iou_thres_list).index(iou_pose_thres) # find the index of 0.1 in iou_th_list

                iou_thres_pred_match = iou_cls_pred_match[thres_ind, :] # find matched gt idx at iou_th = 0.1

                
                cls_pred_class_ids = cls_pred_class_ids[iou_thres_pred_match > -1] if len(iou_thres_pred_match) > 0 else np.zeros(0) # get the pred idx for class_id where there is a matched gt
                cls_pred_RTs = cls_pred_RTs[iou_thres_pred_match > -1] if len(iou_thres_pred_match) > 0 else np.zeros((0, 4, 4))
                cls_pred_scores = cls_pred_scores[iou_thres_pred_match > -1] if len(iou_thres_pred_match) > 0 else np.zeros(0) # repick high iou > 0.1 predictions
                # cls_pred_bboxes = cls_pred_bboxes[iou_thres_pred_match > -1] if len(iou_thres_pred_match) > 0 else np.zeros((0, 4)) # zyh


                iou_thres_gt_match = iou_cls_gt_match[thres_ind, :] # done the same thing on the gt side
            ...
            RT_overlaps = compute_RT_overlaps(cls_gt_class_ids, cls_gt_RTs, cls_gt_handle_visibility, # here theta and translation diff are computed
                                              cls_pred_class_ids, cls_pred_RTs,
                                              synset_names) # (n_pred,n_gt,2) 2: 2 for rotation and translation shifts


            pose_cls_gt_match, pose_cls_pred_match = compute_match_from_degree_cm(RT_overlaps, # (n_deg_th,n_t_th,n_gt/n_pred) use theta and translation to compute matches
                                                                                  cls_pred_class_ids, 
                                                                                  cls_gt_class_ids, 
                                                                                  degree_thres_list, 
                                                                                  shift_thres_list) # outputs: >-1 if the pred and gt pair have shifts both < deg_th, and < t_th in decreasing order of overlap
            

            pose_pred_matches_all[cls_id] = np.concatenate((pose_pred_matches_all[cls_id], pose_cls_pred_match), axis=-1) # accumulate results for this cls
            
            cls_pred_scores_tile = np.tile(cls_pred_scores, (num_degree_thres, num_shift_thres, 1)) # [n_deg_th,n_t_th,n_scores]
            pose_pred_scores_all[cls_id]  = np.concatenate((pose_pred_scores_all[cls_id], cls_pred_scores_tile), axis=-1)
    ...
    # end of iterating over all results and all classes. overall: compute matches at different threshold levels
    ...
    iou_dict = {}
    iou_dict['thres_list'] = iou_thres_list # linspace(0,1,101)
    for cls_id in range(1, num_classes):
        class_name = synset_names[cls_id]
        print(class_name)
        for s, iou_thres in enumerate(iou_thres_list):
            iou_3d_aps[cls_id, s] = compute_ap_from_matches_scores(iou_pred_matches_all[cls_id][s, :], # iou_3d_aps (n_cls,n_iou_th), input size (n_pred), (n_gt)
                                                                   iou_pred_scores_all[cls_id][s, :], # score is for ordering matches
                                                                   iou_gt_matches_all[cls_id][s, :]) # compute ap (recall*precision) using matches. gt_match is for computing recall. precision is matched prediction percent. recall is matched pred / n_gt    
        ax_iou.plot(iou_thres_list, iou_3d_aps[cls_id, :], label=class_name)
        
    iou_3d_aps[-1, :] = np.mean(iou_3d_aps[1:-1, :], axis=0) # average over classes
    ...
    for i, degree_thres in enumerate(degree_thres_list):                
        for j, shift_thres in enumerate(shift_thres_list):
            ...
            pose_aps[-1, i, j] = np.mean(pose_aps[1:-1, i, j]) # average over classes. i, j for deg and translation shifts
    ...
    for cls_id in range(1, num_classes):
        ...
        plt.imshow(pose_aps[cls_id, :-1, :-1], cmap='jet', interpolation='bilinear') # per class
    ...
    plt.xlabel('translation/cm')
    plt.xlim((shift_thres_list[0], shift_thres_list[-2]))
    plt.imshow(pose_aps[-1, :-1, :-1], cmap='jet', interpolation='bilinear') # average over classes
...
def load_mesh(mesh_path, is_save=False, is_normalized=False, is_flipped=False):
    ...
    if is_normalized:
        ...
        mins = np.amin(vertices, axis=0) # same as shapenet normalization
        diffs = maxs - mins
...
def backproject(depth, intrinsics, instance_mask):
    ...
    idxs = np.where(final_instance_mask) # look at true indices. row idx and col idx
    grid = np.array([idxs[1], idxs[0]]) # [col idx, row idx]
    ...
    z = depth[idxs[0], idxs[1]] # [num_pixel]

    # print(np.amax(z), np.amin(z))
    pts = xyz * z[:, np.newaxis]/xyz[:, -1:] # note xyz not in homo coords so /xyz[:,-1] is a normalization factor. z becomes 1 and then multiplied by depth.
    pts[:, 0] = -pts[:, 0] # not sure why. may be related to how grid is built
    pts[:, 1] = -pts[:, 1] # I looked at the backprojected ptc. camera +x to left +y up so these are indeed flipped.
                           # I also looked at the gt pose. In the gt pose +x right, +y down not flipped
    return pts, idxs
...
def align(class_ids, masks, coords, depth, intrinsics, synset_names, image_path, save_path=None, if_norm=False, with_scale=True, verbose=False):
    ...
    for i in range(num_instances):
        ...
        mask = masks[:, :, i]
        coord = coords[:, :, i, :] # guess: nocs coords
        abs_coord_pts = np.abs(coord[mask==1] - 0.5) # mask==1 should flatten this to [h*w,3] 3 is my guess. below 2* probably because there is no min in nocs coords (only partial view)
        bbox_scales[i, :] = 2*np.amax(abs_coord_pts, axis=0) # max along 0 for each later axis. note abs above, this seems similar to shapenet normalization scale max - min

        pts, idxs = backproject(depth, intrinsics, mask)
        coord_pts = coord[idxs[0], idxs[1], :] - 0.5

        if if_norm: # in detect_eval.py set to False (i.e. not set use default)
        ...
        try:
            ...
            scales, rotation, translation, outtransform = estimateSimilarityTransform(coord_pts, pts, False) # scales in x y z are the same values

            aligned_RT = np.zeros((4, 4), dtype=np.float32) 
            if with_scale: # True in detect_eval.py (i.e. not set use default)
                aligned_RT[:3, :3] = np.diag(scales) / 1000 @ rotation.transpose() # this is just scale / 1000 but here scales are x,y,z so a diag mat but here the scales along x,y,z are the same
            else:
                aligned_RT[:3, :3] = rotation.transpose()
            aligned_RT[:3, 3] = translation / 1000 # translation is factored by 1000
            aligned_RT[3, 3] = 1
            
            if save_path is not None:
                coord_pts_rotated = aligned_RT[:3, :3] @ coord_pts.transpose() + aligned_RT[:3, 3:] # transformed coord points Rp+t. not sure usable
    ...
    return RTs, bbox_scales, error_messages, elapses # note scales (single scale) is also factored into the rotation

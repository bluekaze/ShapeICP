...
class NOCSDataset(utils.Dataset):
    ...
    def __init__(self, synset_names, subset, config=Config()): # synset_names: BG, bottle, bowl, etc. subset: val, test (real test)
        self._image_ids = []
        self.image_info = []
        ...
        for i, obj_name in enumerate(synset_names):
            if i == 0:  ## class 0 is bg class
                continue
            self.add_class("BG", i, obj_name)  ## class id starts with 1 # added to self.class_info why always 'BG' as source?

    def load_camera_scenes(self, dataset_dir, if_calculate_mean=False): # use default False, CAMERA dataset only
        ...
        image_dir = os.path.join(dataset_dir, self.subset) # subset = 'val'
        source = "CAMERA"
        num_images_before_load = len(self.image_info) # 0

        folder_list = [name for name in os.listdir(image_dir) if os.path.isdir(os.path.join(image_dir, name))] # 6 digits folder
        
        num_total_folders = len(folder_list)

        image_ids = range(10*num_total_folders) # camera_val25k has 10 images per folder
        color_mean = np.zeros((0, 3), dtype=np.float32) # empty array due to 0
        # Add images
        for i in image_ids:
            image_id = int(i) % 10
            folder_id = int(i) // 10
            ...
            color_path = image_path + '_color.png' # path to the color image
            if not os.path.exists(color_path):
                continue
            
            meta_path = os.path.join(image_dir, '{:05d}'.format(folder_id), '{:04d}_meta.txt'.format(image_id)) # a txt meta file
            inst_dict = {}
            with open(meta_path, 'r') as f:
                for line in f:
                    ...
                    inst_dict[inst_id] = cls_id # also there is the exact object id in meta.txt (looks like shapenet id)
            ...
            self.add_image(
                ...
                width=width,
                height=height,
                inst_dict=inst_dict) # append to a list self.image_info

            if if_calculate_mean: # False
    ...
    def load_real_scenes(self, dataset_dir):
        ...
        source = "Real"
        num_images_before_load = len(self.image_info) # may contain something if already loaded another dataset

        folder_name = 'train' if self.subset == 'train' else 'test'
        image_dir = os.path.join(dataset_dir, folder_name)
        folder_list = [name for name in glob.glob(image_dir + '/*') if os.path.isdir(name)] # scene_x
        folder_list = sorted(folder_list)

        image_id = 0
        for folder in folder_list:
            image_list = glob.glob(os.path.join(folder, '*_color.png'))
            image_list = sorted(image_list) # get all color images
            ...
            for image_full_path in image_list:
                ...
                self.add_image(
                    ...
                    width=width,
                    height=height,
                    inst_dict=inst_dict) # append to self.image_info
                image_id += 1
    ...
    def load_objs(self, image_id, is_normalized): # two commented use of this function both is_normalized=True since scale is estimated separately
        info = self.image_info[image_id]
        ...
        Vs = []
        Fs = []
        for i, line in enumerate(lines): # iterate instances
            words = line[:-1].split(' ')
            inst_id = int(words[0])
            ...
            if len(words) == 3: ## real data # see process_data comments
                if words[2][-3:] == 'npz':
                    ...
                    mesh_file = os.path.join(self.config.OBJ_MODEL_DIR, 'real_val', obj_name) # not my case
                else:
            ...
            # vertices, faces = utils.load_mesh(mesh_file, is_normalized, flip_flag) # zyh
            vertices, faces = utils.load_mesh(mesh_file, False, is_normalized, flip_flag) # zyh
            Vs.append(vertices)
            Fs.append(faces)
    ...
    def process_data(self, mask_im, coord_map, inst_dict, meta_path, load_RT=False):
        ...
        with open(meta_path, 'r') as f: # meta.txt in the data folder
            lines = f.readlines()

        scale_factor = np.zeros((len(lines), 3), dtype=np.float32)
        for i, line in enumerate(lines): # iterate each line = each instance
            words = line[:-1].split(' ')
            
            if len(words) == 3: # inst id, cls id, obj name
                ## real scanned objs # real data
                if words[2][-3:] == 'npz':
                    npz_path = os.path.join(self.config.OBJ_MODEL_DIR, 'real_val', words[2]) # only for real_val not used in my case
                    with np.load(npz_path) as npz_file:
                        scale_factor[i, :] = npz_file['scale']
                else:
                    bbox_file = os.path.join(self.config.OBJ_MODEL_DIR, 'real_'+self.subset, words[2]+'.txt')
                    scale_factor[i, :] = np.loadtxt(bbox_file) # just the txt file with three numbers

                scale_factor[i, :] /= np.linalg.norm(scale_factor[i, :]) # normalized scale factors. why?

            else: # len(words) = 4: inst id, cls, id, obj category shapenet id, obj id
        ...
        for inst_id in inst_dict.keys():
            if inst_dict[inst_id] == 0 or (not inst_id in instance_ids): # so no inst_id in inst_dict is outside the loop below
                inst_id_to_be_deleted.append(inst_id)
        for delete_id in inst_id_to_be_deleted:
            del inst_dict[delete_id]


        for inst_id in instance_ids: # instance_ids is sorted see above. all inst_id in inst_dict is in instance_ids (>= inst_dict)
            if not inst_id in inst_dict: # should not contain deleted instances
                continue # inst_dict == instance_ids
            inst_mask = np.equal(cdata, inst_id) # instance mask is one-indexed (the mask that labels with ints)
            ...
            scales[i, :] = scale_factor[inst_id - 1, :] # pick the ones in inst_id (1-indexed) need 0-indexed index to index scale_factor
            i += 1

        # print('before: ', inst_dict)

        masks = masks[:, :, :i] # since the size is defined as num_instance above. Here clip the trailing entries due to deleted instances
        coords = coords[:, :, :i, :]
    ...
    def load_mask(self, image_id):
        ...
        if info["source"] in ["CAMERA", "Real"]:
            ...
            # return mask the channel is inst id. class_ids is a dict mapping inst_id to cls_id

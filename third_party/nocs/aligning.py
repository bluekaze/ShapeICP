...
def estimateSimilarityTransform(source: np.array, target: np.array, verbose=False):
    ...
    Scales, Rotation, Translation, OutTransform = estimateSimilarityUmeyama(SourceInliersHom, TargetInliersHom) # scales are three same values
...
def estimateSimilarityUmeyama(SourceHom, TargetHom):
    ...
    varP = np.var(SourceHom[:3, :], axis=1).sum() # scalar due to .sum()
    ScaleFact = 1/varP * np.sum(D) # scale factor (scalar)
    Scales = np.array([ScaleFact, ScaleFact, ScaleFact]) # same scale in x,y,z

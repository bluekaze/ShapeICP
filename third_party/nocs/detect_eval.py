...
if __name__ == '__main__':
    ...
    coco_cls_ids = []
    for coco_cls in class_map: # iter keys
        ind = coco_names.index(coco_cls) # find the index of the coco_cls in the list
    ...
    if mode == 'detect':
        ...
        if data == 'val':
            ...
            dataset_val.load_camera_scenes(camera_dir) # load image id, path, instance and class id
            dataset_val.prepare(class_map) # mapping class ids to internal ids. Need run to see what is going on
            dataset = dataset_val
        elif data == 'real_test':
        ...
        image_ids = dataset.image_ids # all image from 0 to N
        save_per_images = 10
        ...
        if mode != 'eval':
            for i, image_id in enumerate(image_ids):
                ...
                result['gt_RTs'] = None            
                result['gt_scales'] = gt_scales # normalized x,y,z scales (norm=1) to build 3d box in 3d iou match (can also build using single scale) probably from shape
                ...
                else: # I think for REAL and CAMERA VAL, there is gt, so this part is not run
                ...
                ## detection
                start = time.time()
                detect_result = model.detect([image], verbose=0) # see model.py/unmold_detections for the outputs
                r = detect_result[0]
                ...
                result['pred_RTs'] = None   # detect only outputs nocs map
                ...
                start = time.time()
                result['pred_RTs'], result['pred_scales'], error_message, elapses =  utils.align(r['class_ids'], # RANSAC + UMEYAMA on each instance RT: [n_inst,:,:]
                                                                                        r['masks'], # used to mask out unrelated pixels
                                                                                        r['coords'], 
                                                                                        depth, 
                                                                                        intrinsics, 
                                                                                        synset_names, 

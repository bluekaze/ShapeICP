...
def compute_3d_iou_new(RT_1, RT_2, scales_1, scales_2, handle_visibility, class_name_1, class_name_2):
    ...
    def asymmetric_3d_iou(RT_1, RT_2, scales_1, scales_2):
        ...
        noc_cube_2 = get_3d_bbox(scales_2, 0) # get box from scale
        bbox_3d_2 = transform_coordinates_3d(noc_cube_2, RT_2) # transform to 3d
...
#         print(np.linalg.det(R1), np.linalg.det(R2))
    # these are all for rotation symmetry not for mirrored symmetry
    if synset_names[class_id] in ['bottle', 'can', 'bowl']:  ## symmetric when rotating around y-axis
        y = np.array([0, 1, 0])
        y1 = R1 @ y
        y2 = R2 @ y # rotate y axis and see the dot product between the rotated y (ideally aligned)
        theta = np.arccos(y1.dot(y2) / (np.linalg.norm(y1) * np.linalg.norm(y2)))
    elif synset_names[class_id] == 'mug' and handle_visibility==0:  ## symmetric when rotating around y-axis. handle is noun. mug handle is visible or not.
        y = np.array([0, 1, 0])
        y1 = R1 @ y
        y2 = R2 @ y
        theta = np.arccos(y1.dot(y2) / (np.linalg.norm(y1) * np.linalg.norm(y2)))
    elif synset_names[class_id] in ['phone', 'eggbox', 'glue']: # zyh edits added chair. REMOVED chair on 20230709 (chair not rotation symmetric)
        y_180_RT = np.diag([-1.0, 1.0, -1.0]) # this rotates object back to front
        R = R1 @ R2.transpose()
        R_rot = R1 @ y_180_RT @ R2.transpose() # first rotate object back to front (180 deg) then apply R1
...
def compute_3d_matches(gt_class_ids, gt_RTs, gt_scales, gt_handle_visibility, synset_names,
    ...
    if num_pred:
        pred_boxes = trim_zeros(pred_boxes).copy() # trim leading and trailing zeros
    ...
    for s, iou_thres in enumerate(iou_3d_thresholds):
        for i in range(len(pred_boxes)): # for each pred box
            # Find best matching ground truth box
            # 1. Sort matches by score
            sorted_ixs = np.argsort(overlaps[i])[::-1] # iou with all the gt boxes
            # 2. Remove low scores
            low_score_idx = np.where(overlaps[i, sorted_ixs] < score_threshold)[0]
            if low_score_idx.size > 0:
                sorted_ixs = sorted_ixs[:low_score_idx[0]]
...
def compute_ap_from_matches_scores(pred_match, pred_scores, gt_match):
    ...
    precisions = np.cumsum(pred_match > -1) / (np.arange(len(pred_match)) + 1) # 1 indexed arange
    ...
    for d, degree_thres in enumerate(degree_thres_list):                
        for s, shift_thres in enumerate(shift_thres_list):
            for i in range(num_pred):
                # Find best matching ground truth box
                # 1. Sort matches by scores from low to high
                sum_degree_shift = np.sum(overlaps[i, :, :], axis=-1) # for each gt, sum the deg and translation shifts
...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False):
    ...
    for progress, result in enumerate(final_results):
        ...
        pred_scores = result['pred_scores'] # some sort of score?
        ...
        for cls_id in range(1, num_classes):
            ...
            if len(iou_pred_indices):
                ...
                cls_pred_bboxes = cls_pred_bboxes[iou_pred_indices] # order things in the same order as internally in compute_3d_matches
            ...
            if use_matches_for_pose: # True
                ...
                cls_pred_bboxes = cls_pred_bboxes[iou_thres_pred_match > -1] if len(iou_thres_pred_match) > 0 else np.zeros((0, 4)) # repick high iou > 0.1 predictions
            ...
            pose_cls_gt_match, pose_cls_pred_match = compute_match_from_degree_cm(RT_overlaps, # (n_deg_th,n_t_th,n_gt/n_pred)
                                                                                  cls_pred_class_ids, 
                                                                                  cls_gt_class_ids, 
                                                                                  degree_thres_list, 
    ...
    # end of iterating over all results and all classes
    ...
    for cls_id in range(1, num_classes):
        ...
        for s, iou_thres in enumerate(iou_thres_list):
            iou_3d_aps[cls_id, s] = compute_ap_from_matches_scores(iou_pred_matches_all[cls_id][s, :], # iou_3d_aps (n_cls,n_iou_th), input size (n_pred), (n_gt)
                                                                   ...
                                                                   iou_gt_matches_all[cls_id][s, :]) # compute ap (recall*precision) using matches. gt_match is for computing recall.    
